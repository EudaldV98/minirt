...
/*   Updated: 2020/01/10 16:59:29 by jvaquer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	ft_for_each_cy(t_ray ray, t_vect3 *p, t_vect3 *n)
{
	...
	while (1)
	{
		hit = ft_intersection_ray_cy(ray, g_data->cy, p, n);
		if (hit > 0)
...
double	ft_intersection_ray_cy_2(t_ray ray, t_vect3 hty, t_vect3 abc,
		t_vect3 cardoc)
{
	hty.x = sqrt(hty.x);
								...
								t_vect3 *p, t_vect3 *n)
{
	...
	if (hty.x < EPS)
		return (0);
	return (ft_intersection_ray_cy_2(ray, hty, abc, cardoc));
}
